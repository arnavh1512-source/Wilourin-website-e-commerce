...
export default function LoginPage() {
  ...
  const handleForgot = async () => {
    if (!forgotEmail) { addToast('Enter your email first', 'warning'); return }
    const supabase = createClient()
        ...
        {forgotMode ? (
          <div className="space-y-4">
            ...
            <input type="email" value={forgotEmail} onChange={(e) => setForgotEmail(e.target.value)}
              className="w-full border border-gray-200 px-4 py-3 text-sm outline-none focus:border-gray-400" placeholder="your@email.com" />
            <button onClick={handleForgot} className="w-full bg-[#0A0A0A] text-white py-3 text-xs uppercase tracking-widest">Send Reset Link</button>
            <button onClick={() => setForgotMode(false)} className="w-full text-sm text-gray-500 underline">← Back to login</button>
          </div>
        ) : (
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              <label className="text-xs uppercase tracking-widest text-gray-500 block mb-1.5">Email</label>
              <input {...register('email')} type="email" className="w-full border border-gray-200 px-4 py-3 text-sm outline-none focus:border-gray-400 transition-colors" placeholder="your@email.com" />
            ...
            <button type="button" onClick={() => setForgotMode(true)} className="text-xs text-gray-400 underline float-right -mt-2">Forgot password?</button>
